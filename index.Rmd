---
title: "The H-1B Visa Applications Project"
author: "Xingyuan Chen, Xin He, Yunzhi Qian, Stephanie Zhen and Katie Gao"


---

<center>

<img src="./picture/H1B homepage.jfif" style="width:75%">

</center>


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

The H-1B is the most common visa US employers use to bring skilled foreign workers to the states. Visas are awarded to employers on a first-come, first-served basis, with applications open to submission beginning in April. If the number of applications exceed an annual cap set by Congress during the first five business days of April, visas are then awarded through a lottery system. 

<center>

```{r echo = FALSE, message = FALSE}
library(tidyverse)
library(viridis)
library(plotly)

options(digits = 2)

h1b_petition = tibble(
  year = c(2012:2018),
  petition_number = c(307713, 299467, 318824, 348669, 398718, 403675, 418799),
  approval_number = c(262569, 286773, 315857, 275317, 345262, 365682, 332358),
  denial_rate = 1 - (approval_number / petition_number),
  denial_rate_percent = denial_rate * 100
)

h1b_petition %>% 
  plot_ly(x = ~year, y = ~petition_number, name = 'number of h1b petition filed', type = 'scatter', mode = 'lines+markers') %>% 
  add_trace(y = ~approval_number, name = 'number of h1b peptition approved ', mode = 'lines+markers') %>% 
  layout(
    xaxis = list(title = 'fiscal year'),
    yaxis = list(
      title = ' ',
      range = c(0,500000)),
    legend = list(x = 0.05, y = 1)
  )

```

</center>

Since 2017, as part of its efforts to “hire American,” the Trump administration has planned to further restrict access to American visas, which has caused the U.S. Citizenship and Immigration Services to more strictly regulate and review H-1B applications. This may explain the decreasing number of approved H-1B visa applications in 2018, even as the number of H-1B petitions filed has risen. 

Since the number of H1-B petitions filed each year is directly shaped by employer demand for work, which is influenced by the strength of the economy, the increasing number of yearly H-1B visa applicants may indicate that the current political environment has not had a large influence on employer's interests to sponsor alien workers.


<img src="./picture/Buy-American-Hire-American.jpg" style="width:75%">



<h2>Additional Links</h2> 
<br>
Here are some additional links about the H-1B visa for those interested in learning more. 

[Everything you need to know](https://www.upcounsel.com/h1b-visa-status) about the H-1B visa: this answers questions related to what it is, how much it costs, who qualifies, as well as pros and cons.

[Key-facts](https://www.pewresearch.org/fact-tank/2017/04/27/key-facts-about-the-u-s-h-1b-visa-program/) of the H-1B visa: more statistics!

[Employer data hub](https://www.uscis.gov/h-1b-data-hub): this includes data from fiscal year 2009 through the third quarter of fiscal year 2019 on employers who have submitted petitions to employ H-1B nonimmigrant workers.


<h2>Screencast</h2> 
<br>
For a brief overview of our website, please view our screencast [here](). 
