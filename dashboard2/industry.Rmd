...
h1b_soc_ind = h1b_soc %>%
  ...
  mutate(soc_code_gen = case_when(
    ...
    soc_code_new >= 130000 & soc_code_new < 140000 ~"Business",
    soc_code_new >= 150000 & soc_code_new < 160000 ~ "Comp, Math",
...
### H-1B Applicantions by Industry
...
### Industries and wage distribution of certified H1B applicants
...
### Top 15 companies who has the most H1B Visa Workers
...
treemap = 
  h1b_soc_con %>% 
  treemap(index = c("employer_name"),vSize = "total", vColor = "total",
              type = "value")


highchart() %>% 
  hc_add_series_treemap(treemap, allowDrillToNode = TRUE,
